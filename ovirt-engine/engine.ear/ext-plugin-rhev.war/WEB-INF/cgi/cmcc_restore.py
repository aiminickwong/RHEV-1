...
class RedhatCmccRestore(object):
    ...
    def get_branch_vm_no(self, vmName):
        """
        """
        allVmObjList = self.api.vms.list()
        vmNameList = [ vo.get_name() for vo in allVmObjList ]
        flag = PREFIX
        targetVmList = []
    ...
    def rcr_create_vm_by_snap(self, vm_name,snap_name,sync=1):
        ...
        if vm_name.startswith(PREFIX):
            ...
            vm_name = l[1]
        vm_name_new = '%s%s%s%s'% (PREFIX,vm_name,PREFIX, self.get_branch_vm_no(vm_name))
	...
	#snapID = snapID
        ...
        sqlcmd = "select memory_volume \
                                  from snapshots \
                                  where snapshot_id='%s'"%snapID
