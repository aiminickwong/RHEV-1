...
class SnapshotHandler(object):
    ...
    def ssh_restore_snap(self, vmName, snapName):
        ...
        snapMap = rcs.cs_get_snapMap(vmName)
...
########################################################################################
    ...
    def ssh_get_volumeID(self, vmName, snapName):
        ...
        volumeID = self._callDbCmd(sqlcmd).dictresult()[0].get('image_guid')
    ...
    def ssh_get_actVolPath(vmName, snapName):
        """
        """
        snapID = self.ssh_get_snapObj(vmName, snapName).get_id()

        cmd = "select * from images where vm_snapshot_id='%s'" % snapID
        imageID = self._callDbCmd(sqlcmd).dictresult()[0].get('image_group_id')
        ...
        poolID = self._callDbCmd(sqlcmd).dictresult()[0].get('storage_pool_id')
...
def get_option():
    ...
    def __test(self,vmName, snapName):
       print self.ssh_get_actVolPath(vmName, snapName)
